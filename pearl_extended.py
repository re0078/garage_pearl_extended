...
class PEARLExtended(PEARL):
    ...
    def __init__(self,
        ...
        super().__init__(env,
                         ...
                         vf_lr,
                         context_lr,
                         policy_mean_reg_coeff,
                         policy_std_reg_coeff,
                         policy_pre_activation_coeff,
                         soft_target_tau,
                         kl_lambda,
                         optimizer_class,
        ...
        self.fepi_alpha = fepi_alpha
        self._state_model = state_model
        self.state_optimizer = optimizer_class(
            self._state_model.parameters(),
            lr=state_model_lr,
        )
    ...
    def _optimize_policy(self, indices):
        ...
        state_pred = self._state_model(torch.cat([obs, actions], dim=1),
                                       task_z)
        ...
        state_loss = self.fepi_alpha * torch.nn.functional.mse_loss(state_pred,
                                                                    next_obs)
        state_loss.backward()
    ...
    def networks(self):
        ...
        return self._policy.networks + [self._policy] + [
            self._qf1, self._qf2, self._vf, self.target_vf, self._state_model
        ]
